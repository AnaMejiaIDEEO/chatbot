    ...
    methods: {
        ...
        getArbol(nodo_init){ 
            ...
            .then( response => {
                ...
                this.coinAux = response.data.porcentaje
            }).catch( err => {
                console.log(err);
            })           
        },
                ...
                if( v.arbol_final == 0 ){
                    ...
                    .then( response => {
                        ...
                        if( response.data.content == 0 ){
                            ...
                            v.coinAux = response.data.porcentaje
        ...
        validateScore(puntuaciones){
            if( puntuaciones.length > 0 ){
                puntuaciones.forEach(element => {
                    ...
                    if( element.tipo == "Ronda" ){
                        this.puntosRonda = this.puntosRonda + element.valor;
                        console.log("Ronda: "+this.puntosRonda);
                    }
                });
            }
        },
        ...
        updateCoin(nodo){
            this.coin = this.coinAux
            this.metrica(this.recomendacion+"-"+nodo, this.coinAux, 'Avance', 0) 
        },
        validarEstiloRespuesta(opcion_id,correcto){
            console.log("validarEstiloRespuesta: "+opcion_id+"-"+correcto)
            if( correcto !== null){
                let button_id = document.getElementById(opcion_id)
                if( correcto === 1)
                    button_id.style.backgroundColor="#78ff78";
                if( correcto === 0)
                    button_id.style.backgroundColor="#ff7c7c";
            }
        },
    },
    beforeMount(){
